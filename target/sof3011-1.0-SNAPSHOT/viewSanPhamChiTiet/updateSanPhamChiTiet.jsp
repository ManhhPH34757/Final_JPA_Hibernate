...
<div class="row mt-3">
    ...
    <div class="col-3"></div>
    <div class="col-6">
        <form action="update-ctsp?idSp=${sp.id}&idctsp=${ctsp.id}" method="post">
            <div class="mt-3">
            ...
            <div class="mt-3">
                ...
                <select class="form-control" name="idMauSac">
                    ...
                    <c:forEach items="${mauSac}" var="ms">
                        <option value="${ms.id}" <c:if test="${ctsp.idMauSac == ms.id}">selected</c:if> >${ms.tenMau}</option>
                    </c:forEach>
                </select>
            </div>
            <div class="mt-3">
                ...
                <select class="form-control" name="idSize">
                    ...
                    <c:forEach items="${size}" var="s">
                        <option value="${s.id}" <c:if test="${ctsp.idSize == s.id}">selected</c:if> >${s.tenSize}</option>
                    </c:forEach>
                </select>
            </div>
            <div class="mt-3">
                <label for="giaBan"><strong>Giá bán:</strong></label>
                <input class="form-control" name="giaBan" id="giaBan" type="text" value="${ctsp.giaBan}" >
            </div>
            <div class="mt-3">
                <label for="soLuongTon"><strong>Số lượng tồn:</strong></label>
                <input class="form-control" name="soLuongTon" id="soLuongTon" type="text" value="${ctsp.soLuongTon}" >
            </div>
            <div class="mt-3">
